...
/**
 * @brief This is a derived class of `VariableAttributeLoader` where the user
 * implements their variable attributes and field declarations.
 */
...
public:
  ...
  /**
   * @brief User-implemented method where the variable attributes are set for
   * all fields.
   */
  void load_variable_attributes() override;
};

/**
 * @brief This is a derived class of `MatrixFreeOperator` where the user
 * implements their PDEs.
 *
...
private:
   ...
   */
  void set_initial_condition(const unsigned int &index,
   ...
   */
  void set_nonuniform_dirichlet(const unsigned int &index,
  ...
  /**
   * @brief User-implemented class for the RHS of postprocessed explicit
   * equations.
   */
  void compute_postprocess_explicit_rhs(
      VariableContainer<dim, degree, number> &variable_list,
      const dealii::Point<dim, dealii::VectorizedArray<number>> &q_point_loc,
      const dealii::VectorizedArray<number> &element_volume,
      Types::Index solve_block) const override;
  ...
  number delta =
      get_user_inputs().get_user_constants().get_model_constant_double("delta");
  number nu_3 =
      get_user_inputs().get_user_constants().get_model_constant_double("nu_3");
  number nu_4 =
      get_user_inputs().get_user_constants().get_model_constant_double("nu_4");
  number sigma_star =
      get_user_inputs().get_user_constants().get_model_constant_double(
          "sigma_star");
  number k =
      get_user_inputs().get_user_constants().get_model_constant_double("k");
  number sine_coefficient =
      get_user_inputs().get_user_constants().get_model_constant_double(
          "sine_coefficient");
  number E_x_0 =
      get_user_inputs().get_user_constants().get_model_constant_double("E_x_0");
  number E_y_0 =
      get_user_inputs().get_user_constants().get_model_constant_double("E_y_0");
