...
void CustomPDE<dim, degree, number>::compute_nonexplicit_lhs(
  ...
  if (current_index == 0) {
    if constexpr (dim == 3) {
      ...
      // Compute the residual
      ScalarValue residual = f_change - 0.5 * dt * velocity * f__change_grad;

      variable_list.set_value_term(0, residual, Change);
      variable_list.set_gradient_term(
          0, -residual * stabilization_parameter * velocity, Change);
    }
  }
}
