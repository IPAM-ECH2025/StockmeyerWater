...
void write_vtu(const ViewType3D &f, const ViewType1D &y,
               const ViewType1D &theta, const ViewType1D &omega,
               const std::string &filename, RealType time = 0.0,
               int increment = 0, const std::string &output_dir = "outputs/") {
  ...
  const auto _N_theta = P::N_theta;
  const auto _N_omega = P::N_omega;
