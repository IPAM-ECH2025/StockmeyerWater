...
private:
  ...
  number nu3 = get_user_inputs().get_user_constants().get_model_constant_double("nu3");
  number nu4 = get_user_inputs().get_user_constants().get_model_constant_double("nu4");
  number sigma_s =
    get_user_inputs().get_user_constants().get_model_constant_double("sigma_s");
  number k = get_user_inputs().get_user_constants().get_model_constant_double("k");
  number sine_coeff =
    get_user_inputs().get_user_constants().get_model_constant_double("sine_coeff");
  number omega_max =
    get_user_inputs().get_user_constants().get_model_constant_double("omega_max");
  number Ex0 = get_user_inputs().get_user_constants().get_model_constant_double("Ex0");
  number Ey0 = get_user_inputs().get_user_constants().get_model_constant_double("Ey0");
  ...
  ScalarValue compute_stabilization_parameter(const VectorValue &velocity,
                                  const ScalarValue &element_volume) const
  {
    ...
    // Neglecting nu term (dependent on Reynolds number)
    // 9.0 * dealii::Utilities::fixed_power<2>(4.0 * nu / h / h));

    return stabilization_parameter;
  }

    ScalarValue compute_stabilization_parameter_2(const VectorValue &velocity,
                                  const ScalarValue &element_volume) const
  {
    // Norm of the local velocity
    ScalarValue u_l2norm = 1.0e-12 + velocity.norm_square();
    ...
    ScalarValue h = std::pow(element_volume, ScalarValue(1.0/3.0));;
    //Element diagonal
    ScalarValue diag = std::sqrt(3.0)*h;

    ScalarValue stabilization_parameter = diag/(2.0*(degree+1.0)*std::sqrt(u_l2norm));
